...
def batalha(inimigo):
    ...
    while vida_jogador > 0 and vida_inimigo > 0:
        ...
        if escolha == "1":
            ...
            elif poções >= 0 and vida_jogador <= vida_maxima:
                cura = random.randint(10, 20)
                vida_jogador += cura
                poções -= 1
                sleep(1)
                print(f"\n🧪💔 Você usou uma poção e recuperou {cura} de vida! 💖")
                if vida_jogador + cura > vida_maxima:
                    vida_jogador = 0 + vida_maxima
                    print("Sua vida está no máximo! 🔥")

                else:
                    continue
                
            else:
                print("Você não pode usar essa poção... Sua vida já está cheia.")
